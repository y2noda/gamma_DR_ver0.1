...
#     
  ...
  # 通常のGLM
  # r <- glm(YY ~ XX, family = binomial)
  # r$coefficients
  # res_df <- r$coefficients %>% data.frame() %>% na.omit %>% t
  # colnames(res_df) <- c("b1","b2","b3","b4","b5","b6","b7","b8","b9")
  # 
  # results.beta_hat[i,] <- res_df
  ...
  #   pi <- ei_gamma/(1+ei_gamma)
  #   S <- t(XX)%*%diag(as.vector(wi/n)) %*% (YY-pi)
  #   return(S)
  # }
  # 
  # gamma <- 2
